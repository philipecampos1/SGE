...
def get_graphic_product_category_metric():
    ...
    return {category.name: Product.objects.filter(category=category).count() for category in categies}


def get_graphic_product_brand_metric():
    brands = Brands.objects.all()
    return {brand.name: Product.objects.filter(brands=brand).count() for brand in brands}
